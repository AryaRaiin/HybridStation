...
public sealed class DroppableBorgModuleSystem : EntitySystem
{
    ...
    private void OnModuleSelected(Entity<DroppableBorgModuleComponent> ent, ref BorgModuleSelectedEvent args)
    {
        ...
        for (int i = 0; i < ent.Comp.Items.Count; i++)
        {
            ...
            // HS START
            if (!_hands.TryGetHeldItem(chassis, handId, out var heldItem))
            {
                Log.Error($"Failed to pick up {ToPrettyString(item)} into hand {handId} of {ToPrettyString(chassis)}, it holds nothing");
                _hands.RemoveHand(chassis, handId);
            }
            else if (heldItem.Value != item)
            {
                Log.Error($"Failed to pick up {ToPrettyString(item)} into hand {handId} of {ToPrettyString(chassis)}, it holds {ToPrettyString(heldItem.Value)}");
                _hands.RemoveHand(chassis, handId);
            }
            else
            {
                _interaction.DoContactInteraction(chassis, item);
